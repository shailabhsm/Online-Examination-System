...
$marks=0;
$attempt=0;
$sql = "SELECT * FROM res WHERE e_no=".$e;
...
$sql = "SELECT *FROM ".$subject." WHERE q_no=".$i;
$query=mysqli_query($link,$sql);
if($query){
			while($row=mysqli_fetch_array($query))
				{
...
$n_attempt = $t_ques - $attempt;
$percent = round(($marks*100)/$t_ques,2);
if($percent <= 0)
{
	$percent = 0;
}
...
echo '<p>Total Question<br><font color="#fff">'.$t_ques.'</font></p>';
...
<?php
$q=0;
$sql = "SELECT * FROM ".$subject." LIMIT ".$t_ques;
$query=mysqli_query($link,$sql);
if($query)
{
	while($row=mysqli_fetch_array($query))
